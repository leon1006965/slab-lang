...
def compile_image(node: Node) -> list:
    ...
    lines.append(f'        _photo = ImageTk.PhotoImage(_orig)')
    lines.append(f'        _img_label = tk.Label(root, image=_photo)')
    lines.append(f'        _img_label.pack(padx=10, pady=10)')
    lines.append(f'        root._img = _photo')
    lines.append(f'        root._orig_img = _orig')
    lines.append(f'        def _resize_img(event):')
    lines.append(f'            _w = max(event.width - 20, 1)')
    lines.append(f'            _h = max(event.height - 20, 1)')
    lines.append(f'            _r = _orig.copy().resize((_w, _h))')
    lines.append(f'            _ph = ImageTk.PhotoImage(_r)')
    lines.append(f'            _img_label.config(image=_ph)')
    lines.append(f'            _img_label.image = _ph')
    lines.append(f'        root.bind("<Configure>", _resize_img)')
